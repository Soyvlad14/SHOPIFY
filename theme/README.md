...
## Quitar logo de OXXO + íconos de franja de confianza (tema borrador `Paulina Montiel - Quitar Logo OXXO`)
...
- `snippets/product-social-proof.liquid` — los íconos de "Garantía de devolución de 30 días" y "Empresa Familiar Mexicana" dejan de ser SVG dibujados a mano y pasan a ser imágenes reales (`assets/pt-devolucion-icon.png`, `assets/pt-bandera-mexico.png`, generadas y subidas al tema vía la API con `body: { type: URL }`), pegadas dentro de la burbuja circular con `object-fit: cover`, igual que el resto de los badges
